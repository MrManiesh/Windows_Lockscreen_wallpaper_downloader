...
root.title('Windows 10 lock screen wallpaper downloader by @Expert.py')
def downloadImage():
    try:
        for selected_item in lst.selection():
            item = lst.item(selected_item)
        item['values'][0]
        shutil.copy2(r"{}\expertdotpy\{}".format(tempfile.gettempdir(),item['values'][0]), r"C:\Users\{}\Downloads".format(os.getenv('username')))
...
label1 = tk.Label(root, text="@expert.py",fg = "black",bg = "white",font=("Raleway", 20, "bold"))
label1.grid(row = 0, column = 0) 

label1 = tk.Label(root, text="Windows 10 lockscreen wallpaper downloader",fg = "white",bg = "black",font=("Raleway", 20, "bold"))
